...
BOT_NAME = os.environ.get("BOT_NAME", "Task Bot")
...
async def add_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db_pool.acquire() as conn:
        ...
        """,
            ...
            email,
            data.get("password", ""),
            data.get("year of birth", "")
        )

    if result == "INSERT 0 1":
        await update.message.reply_text(f"✅ Task added: {email}")
    else:
        await update.message.reply_text(f"⚠️ Email already exists: {email}")
